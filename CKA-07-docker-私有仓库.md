...
docker tag centos:v1 192.168.26.32:5000/cka/centos:v1

docker push 192.168.26.32:5000/cka/centos:v1
...
OPTIONS='--selinux-enabled --log-driver=journald --signature-verification=false --insecure-registry=192.168.26.32:5000'
...
#查看仓库中有哪些镜像(json_reformat格式化JSON)：
$ curl http://192.168.26.32:5000/v2/_catalog ｜json_reformat

#jq也可以格式化JSON(推荐jq)
$ yum install jq -y
$ curl http://192.168.26.32:5000/v2/_catalog ｜jq

# 查看镜像全部tag:
$ docker push 192.168.26.32:5000/rhce/busybox 
$ curl -XGET http://192.168.26.70:5000/v2/rhce/nginx/tags/list
```

**修改docker默认仓库地址`docker.io`**

修改默认地址
```shell
#增加ADD_REGISTRY 参数
$ vim /etc/sysconfig/docker
ADD_REGISTRY='--add-registry 192.168.26.32:5000'
$ systemctl restart docker
```
...
$ /usr/local/bin/delete_docker_registry_image -i db/mysql:v2
```

<details>
  <summary>delete_docker_registry_image 脚本</summary>
